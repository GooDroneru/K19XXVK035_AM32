...
{   
    ...
    if ((diffValue > 800) && (diffValue < 2200))
    {
        ...
        if (calibration_required)
        {
            if (!high_calibration_set)
            {
                ...
                {
                servo_high_threshold = ((7 * servo_high_threshold + diffValue) >> 3);
            ...
            if (high_calibration_set)
            {
                if (diffValue < 1250)
                {
                    ...
                    servo_low_threshold = ((7 * servo_low_threshold + diffValue) >> 3);
				}
    ...
    (newinput - servorawinput > max_servo_deviation)
    {
		newinput -= max_servo_deviation;
    }
    else
...
{   
    ...
    if (inputSet == 1)
    {
        ...
        if (!armed)
        {
            ...
            {   // note this in input..not newinput so it
