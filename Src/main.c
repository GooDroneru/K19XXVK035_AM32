...
//============================= Servo Settings ==============================
uint32_t servo_low_threshold = 1100; // anything below this point considered 0
uint32_t servo_high_threshold = 1900; // anything above this point considered 2000 (max)
uint32_t servo_neutral = 1500;
uint32_t servo_dead_band = 100;
...
uint16_t minimum_duty_cycle = 0;
uint16_t stall_protect_minimum_duty = 0;
char desync_check = 0;
...
{  // 20khz as of 2.00 to be renamed
	...
	//				send_LED_RGB(255, 0, 0);
                ...
                { // 1khz PID loop
                    ...
                    {
                        use_current_limit_adjust -= (int16_t)(doPidCalculations(&currentPid, actual_current, eepromBuffer.limits.current * 2 * 100) / 10000);
...
{ // only used in polling mode, blocking routine.
	...
	COM_TIMER->VALUE = 0xFFFFFFFF - waitTime;
    commutate();
    bemfcounter = 0;
    bad_count = 0;
    zero_crosses++;
...
int main()
{
 ...
 //	}
    ...
    if (stepper_sine == 0)
    {
        ...
        {                                   // some hardware doesn't need this, its on
                                          // by default to keep hardware / motors
                                          // protected but can slow down the response
                                          // in the very low end a little.
            duty_cycle_maximum = map(k_erpm, low_rpm_level, high_rpm_level, throttle_max_at_low_rpm, throttle_max_at_high_rpm); // for more performance lower the
